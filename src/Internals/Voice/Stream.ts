...
class Stream {
    ...
    async init() {
        ...
        let threshold =
            (await this.connection.channel.guild.threshold) === "high"
                ? 2000
                : (await this.connection.channel.guild.threshold) === "medium"
                ? 1750
                : 1500;
        ...
        this.stream.on("data", async (data: any) => {
            ...
            if (Stream.getVolume(data) > threshold) {
                ...
                if (
                    this.connection.guildConnection.cases.get(this.member.id).filter((x: any) => x === punishment)
                        .length < 2
                ) {
                    ...
                    let notifyChannel = this.member.guild.channels.cache.get(
                        await this.member.guild.notifyChannel
                    ) as TextChannel;
                    if (notifyChannel) {
                        notifyChannel.send(
                            new warnEmbed(
                                `A member has been warned for high mic volume.\nPerson: ${this.member.user} (${this.member.id})`
                            )
                        );
                    }
                    return this.member.send(
                        new warnEmbed(
                            "You are being given your only warning. Audio of too high of a volume is being outputted from your mic. Please adjust your mic volume/ask others to increase your volume. You will be punished if your volume exceeds the threshold again."
                        )
                    );
                }
                switch (punishment) {
                ...
                if (notifyChannel) {
                    notifyChannel.send(
                        new punishEmbed(
                            `A member has been punished for high mic volume.\nPerson: ${this.member.user} (${this.member.id})\nPunishment: ${punishment}`
                        )
                    );
                }
                this.member
                        ...
                        new punishEmbed(
                            `You have been ${
                                punishment === "vc_mute"
                                    ...
                                    : "unknown type"
                            } due to excessive volume coming from your mic. If you did not do this intentionally, please take proper measures to ensure this doesn't happen again. Further punishment may occur if you don't comply.`
                        )
                    )
                    .catch(() => {});
    ...
    static getVolume(buffer: { length: number; readInt16LE: (arg0: number) => number }) {
        let total = 0;
