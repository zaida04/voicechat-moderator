...
export default async (message: Message) => {
    ...
    const prefixRegex = new RegExp(
        `^(<@!?${message.client.user ? message.client.user.id : null}>|${escapeRegex(prefix)})\\s*`
    );
    ...
    if (
        command.permissions &&
        !message.member?.hasPermission(command.permissions, { checkAdmin: true, checkOwner: true })
    ) {
        return message.reply(
            new incorrectUsageEmbed(`Sorry, but you do not have the permissions: ${command.permissions.join(", ")}`)
        );
    }
    ...
    if (command.args && args.length < command.args) {
        return message.channel.send(
            new incorrectUsageEmbed(
                `Sorry, either you didn't provide any arguments or you provided too many! ${
    ...
    try {
        ...
        return message.channel.send(
            `An internal exception has occured. This should not happen, and we'd like to ask you to join our support server that you can access using \`${prefix}support\` and to copy paste the error below and the command code into https://pastebin.com/ and share it with us. \n\n The error to copy paste: \`\`\` ${e} \`\`\`\n **Please Include this: Command Name: ${command.name}**`
        );
    }
};
